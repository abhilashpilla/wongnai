...
public class OrderService {
    ...
    public OrderService(ReactiveOrderRepository orderRepository, OrderMapper orderMapper, SqsMessageProducer producer, ReactiveDriverRepository driverRepository, 
    		OrderRepository lockingOrderRepository, DriverRepository blockingDriverRepositoryy, ReactiveOrderEventRepository reactiveOrderEventRepository, DriverEventRepository driverEventRepository,
    ...
    public Mono<OrderDTO> partialUpdate(OrderDTO orderDTO) {
       ...
       /* .flatMap(ss -> {
    		...
    		OrderEvent ordEvt = new OrderEvent();
    		ordEvt.setStatus(ss.getStatus());
    		ordEvt.setOrderId(ss.getId());
    		ordEvt.setTime(Instant.now());
    		return reactiveOrderEventRepository.save(ordEvt);
    ...
    public Mono<OrderDTO> update(OrderDTO orderDTO) {
       ...
       /* Mono<Order> oo = orderRepository.findById(orderDTO.getId()).map(result -> {
        	
        	Order ord = new Order();
        	ord.setId(result.getId());
    ...
    public void processOrder(OrderDTO orderDto, List<Driver> drivers) throws Exception {
        ...
        //System.out.println("driver soze:"+drivers.size());
        //if(drivers.size() == 0)
        	//throw new Exception();
	    ...
	    try {
	        ...
	        DistanceMatrixRow[] rows = req.origins(locations)
	        ...
	        Optional<Order> orderFound=lockingOrderRepository.findById(orderDto.getId());
	        System.out.println("orderFound:"+orderFound.get());
	        ...
	        //System.out.println("drivers.get(value).getId():"+drivers.get(value).getId());
	        //d.setId(drivers.get(value).getId());
	        
	        
	        //System.out.println(result.map(res-> res.getDriver().getId()).toString());
    ...
    public Optional<OrderDTO> partialUpdateCustom(OrderDTO orderDTO) {
        ...
        return lockingOrderRepository
            ...
            .map(lockingOrderRepository::save)
